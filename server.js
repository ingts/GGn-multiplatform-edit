...
const httpGetStatus = util.promisify((url, cb) =>
  http.get(url, (res) => cb(null, res.statusCode)),
);
  ...
  async ({ options, warnings }) => {
    ...
    if (process.argv.indexOf("--test") !== -1) {
      ...
      setTimeout(async function () {
        ...
        assert.equal(
          await httpGetStatus(`http://localhost:${port}/${devScriptInFile}`),
          200,
          `http://localhost:${port}/${devScriptInFile}`,
        );
        if (outFiles) {
          for (let i = 0; i < outFiles.length; i++) {
            ...
            assert.equal(
              await httpGetStatus(`http://localhost:${port}/${urlPath}`),
              200,
        ...
        console.log(
          italic("Stopping server and watcher after 10 seconds and exiting."),
        );
        watcher.close();
        server.close();
        process.exit();
    ...
    watcher.on("event", (event) => {
      if (event.code === "BUNDLE_START") {
        ...
        outFiles = event.output.map((fullPath) =>
          path.relative(path.resolve(destDir), fullPath),
        );
        console.log(
          green(
            `created ${bold(
